...
| macOS（Apple Silicon / M 系列，需要 macOS 13 及以上） | `Aihub-x.y.z-mac-arm64.dmg` | 拖进「应用程序」即可；另有 `.zip` 版备用 |
...
| 启动时预加载 | **默认关闭**：启动只加载标签栏当前要显示的那几栏。⚙ 里可开启，开启后会把所有标签的页面依次加载一遍（只为了让标签栏显示各站点的真实标题——logo 不开也有） |
| 后台标签自动休眠 | **默认开启**：后台标签闲置超过 30 分钟就释放掉它的页面进程，点回去自动从原来的地址恢复。⚙ 里可调 15 分钟 / 30 分钟 / 2 小时 / 不休眠 |
...
| 自动更新（Windows） | 无需手动：启动后自动在后台查、下载完在底部提示「立即重启」。想主动查：**标签栏空白处右键** →「检查更新」（菜单上会写当前版本 / 下载进度）|
| 中键点击标签 | 重新加载该标签 |
| 网页里右键 | 复制 / 粘贴 / 复制链接 / 复制图片 / 在浏览器打开 / 刷新 / 检查元素（服务页面里也能正常右键了） |
| 标签栏空白处右键 | 刷新、回到首页、切预加载、看当前休眠档位、检查更新、隐藏到后台、设置、开发者工具（菜单里也会写明当前唤出快捷键） |
...
- **应用自己这一层**（标签栏 / 空白处）：渲染层发 `ui:shell-menu`，主进程弹
  「刷新 / 回到首页 / 启动时预加载（勾选）/ 后台标签休眠（只读显示当前档位）/ 外观主题 / 设置 / 开发者工具」。
...
4. **默认只加载标签栏当前要显示的那几栏**：启动时只给分屏里的栏和当前激活的标签建视图，
   其余标签是**懒创建**——点过才占一个渲染进程。标签栏上的名字和 logo 不依赖页面加载（见下一节），
   所以不点开也是全的。
   **启动时预加载**是可选项（⚙ 设置里的开关，默认关闭）：开启后每隔 350ms 建一个服务的视图，
   把所有标签的页面都加载起来（视图**不挂到窗口上**，所以不占显示区域、也不影响分屏），
   这样各站点的真实标题不用点开就会出现——代价是启动即多个渲染进程 + 多次真实网络请求。
5. 没有 `iframe`，所以 `X-Frame-Options`、CSP 的 `frame-ancestors` 都拦不住它——
   这也正是不能直接用 `<iframe>` 的原因。
6. **后台标签闲置自动休眠**（默认开启，30 分钟）：主进程每 60 秒扫一遍，把闲置超过设定时间的
   后台标签的渲染进程整个释放掉（复用删除服务时那套 `destroyView`）。下面这些情况**一律不休眠**：
   在分屏布局里显示着的、当前激活的那一栏、正在出声的（语音/朗读）、正在加载的、
   开着开发者工具的，以及**输入框里还有没发出去的字的**（回收前会往页面里探一次，
   探测超时或出错都按「有输入」处理，宁可多占一会儿内存也不吞掉写了一半的提示词）。
   另外**只要有第三方登录弹窗开着，这一轮整个不回收**——弹窗的回调要落回原标签，
   把标签本体收掉这次登录就白做了。
   点回一个休眠的标签就自动恢复（按 `F5` 刷新、点 ⌂ 回首页同样会把它唤醒）：
   起始地址取自 `config.lastUrls`，也就是**休眠前停在哪儿就回到哪儿**；
   标签栏上的名字来自渲染层自己缓存的标题、图标来自内置本地 logo，
   两者都和页面进程的生死无关，所以整个过程标签栏**不会闪、不会掉图、不会变回默认名**。
...
| 内容 | 位置 |
| --- | --- |
| 服务列表 / 上次打开的标签 / 分屏布局（栏位与宽度）/ 各标签上次的网址 / 预加载开关 / 休眠档位 | `%APPDATA%\Aihub\config.json` |
...
自动更新只认 **GitHub Releases 上的 `latest.yml`**，而 `latest.yml` 是 CI 打包时生成的，
所以发版必须走标签，不能手动传 exe：

```powershell
# 1. 改 package.json 的 version（必须比线上的大，autoUpdater 就是靠这个比的）
# 2. 提交
git commit -am "release: v0.2.0"
# 3. 打标签推上去，剩下的交给 GitHub Actions
git tag v0.2.0
git push origin main --tags
```

CI 会自动出 Windows 安装包，并把 **`.exe` + `.exe.blockmap` + `latest.yml`** 三件一起挂到 Release 上。
装了旧版的人最迟 4 小时内就会收到（开着应用的话）。

三件缺一不可：

| 文件 | 少了会怎样 |
| --- | --- |
| `latest.yml` | 更新源找不到，客户端一直报 404，永远收不到新版本 |
| `*.exe.blockmap` | 差量更新退化成每次全量下载 100MB+ |
| `*.exe` | 下载不到安装包 |

> **最容易踩的坑：忘了改 `version`。** 标签推了、Release 也出来了，但包里的版本号没变，
> 客户端一比「线上和我一样」，于是谁都收不到更新。
>
> **macOS 不发 `latest-mac.yml`**（CI 里是特意只收 `latest.yml` 的）——mac 包没签名，
> Squirrel.Mac 必定校验失败，放出去等于给 mac 用户一个永远装不上的更新源。
...
它会真的打开窗口，依次切换每个标签、真实加载各站点，然后断言：默认只给标签栏要显示的那几栏建视图、
...
后台标签休眠（默认档位、非法档位被拒、关掉休眠后扫描不回收任何视图、闲置标签被回收、
分屏里的和当前激活的标签豁免、有登录弹窗开着时整轮不回收、
**休眠后标签栏的名字和本地 logo 一个都不掉**、
点开后视图重建且回到休眠前的同一站点、对休眠中的标签按刷新会把它唤醒）、
登录信息导出/导入的完整往返（含 HttpOnly/Secure/SameSite 属性、本地存储、加密与错误密码）、
session 隔离与 UA 伪装、标签栏页面无 JS 报错。最后打印 `SMOKE_PASS x/x` 或 `SMOKE_FAIL`，
退出码 0/1，可接 CI。（当前 279 项。）

> 自检碰真实账号的部分**只读**（只导出、只做「不减少」的断言）；
> 清空 Cookie、写本地存储这类破坏性验证只在临时服务上做，做完连分区一起删掉，
> 不会动你实际的登录态。它开跑前会把 `config.json` 整份存下来，结束时原样写回去，
> 所以标签、分栏、主题、快捷键这些设置不会被自检改掉（报告里会印一行「配置还原」）。
...
注意：这会真实访问七个站点。自检过程中配置会被改（为了截浅色那几张图会切到浅色），
但结束时会把开跑前的 `config.json` 原样写回去，不用自己收拾。
...
- **预加载会同时加载所有站点**（**默认已关闭**）：开启后每个标签都是一个真实的渲染进程 +
  一次真实的页面请求。它现在只换来「标签栏显示各站点的真实标题」这一件事——logo 和名字不开也有，
  所以除非你确实在意标题，没必要开。
- **每个仍然活跃的标签就是一个渲染进程**：相当于同时开着好几个浏览器标签页。
  默认只加载标签栏当前要显示的那几栏，后台标签闲置 30 分钟自动回收（⚙ 里可调 15 分钟 /
  30 分钟 / 2 小时 / 不休眠）；但**分屏里显示着的栏永远不会被回收**——同时开 4 栏就是常驻 4 个进程，
  这部分只能靠少开栏或删掉不用的服务。
- **跨域 iframe 里没发送的输入探测不到**：回收前的「输入框里有没有没发出去的字」是在页面主文档里查的，
  少数把输入框放在跨域 iframe 里的站点探不到，极端情况下仍可能连着草稿一起被回收。
  介意的话把休眠调长，或者在 ⚙ 里选「不休眠」。
...
- [x] **7. 自动更新（Windows）**：接入 `electron-updater` + GitHub Releases，启动 10 秒后查一次、
      之后每 4 小时查一次，后台静默下载完在底部提示「立即重启」；不点的话下次退出时自动装上。
      macOS 暂不接——Squirrel.Mac 强制校验代码签名，未签名的包接上去只会每次都失败（见第 13 条）
...
- [x] **14. 内存优化**：后台标签闲置自动休眠（默认开启、30 分钟，可调 15 分钟 / 2 小时 / 不休眠，
      分屏中 / 出声中 / 加载中 / 开着 DevTools / 有未发送输入的标签豁免，登录弹窗开着时整轮不收，
      刷新和回首页会唤醒休眠标签），
      启动预加载默认关闭、改为只加载标签栏当前需要的那几栏
...
| 启动后内存/流量比之前高 | 预加载现在**默认是关的**，启动只加载标签栏要显示的那几栏。如果仍然偏高，先到 ⚙ 里确认「启动时预加载所有标签」没被打开，再看分屏开了几栏（每栏都是一个常驻渲染进程） |
| 某个标签看着变淡了 | 它已经休眠（闲置超过设定时间，页面进程被释放掉了），点一下就自动恢复。嫌它回收太快就到 ⚙ →「后台标签自动休眠」里调长，或者直接选「不休眠」 |
| 点回一个很久没用的标签，页面重新加载了一次 | 同上，这是休眠后的正常恢复。地址会回到你上次停的那一页，但页面内的临时状态（比如没提交的表单）不保证还在；正在编辑的内容不会被回收——回收前会先检查输入框 |
